...
def plot_figure_setup(xlabel='x', ylabel='y', hline=False, vline=False, equal_axes=False):
    f = plt.figure()
    if equal_axes:
        plt.axis('equal')
    plt.xlabel(xlabel)
...
first = np.random.normal(-1, 1, (SAMPLE_NUM // 2, 2))
second = np.random.normal(1.7, 1, (SAMPLE_NUM // 2, 2))

X = np.c_[np.ones(SAMPLE_NUM), np.concatenate((first, second))]
Y = np.r_[np.ones(SAMPLE_NUM // 2), -np.ones(SAMPLE_NUM // 2)].reshape(SAMPLE_NUM)
...
plot_figure_setup(hline=True, vline=True, equal_axes=True)
...
plot_figure_setup('M', 'Q', True, True, True)
plt.plot(margin, list(map(exact_loss_m, margin)), label='$[M<0$]')
plt.plot(margin, list(map(mse_loss_m, margin)), label='$(M - 1)^2$')
plt.plot(margin, list(map(hinge_loss_m, margin)), label='$max(0,1-M)$')
plt.plot(margin, list(map(logistic_loss_m, margin)), label='$\ln(1+e^{-M})$')
plt.legend()
plt.xlim(-7, 7)
plt.ylim(-3, 3)
plt.show()
...
def grad_descent(w, func, grad, learn_rate, iter_num):
    cur = w.copy()
...
# значительно возрастать) с шагом в $10^{-4}$, при этом оптимизировалось значение функции в найденной точке. Логично,
#  что оптимальная скорость обучения близка к 0.5, так как тогда за одну итерацию достигается $(x;y)-0.5\cdot(
# 2x;2y)=(0;0)$ - глобальный минимум
...
plot_figure_setup('Number of iterations', 'Loss')
...
plot_figure_setup('Batch size', 'Accuracy')
...
plot_figure_setup('Batch size', 'Loss')
...
# Такое число проходов для каждого размера батча мы ставим, чтобы в каждом случае алгоритм проработал одно и то же
# число итераций.
...
plot_figure_setup('Iteration', 'Loss')
...
xgrid, ygrid = np.meshgrid(xx, yy)
zgrid = rosenbrock((xgrid, ygrid))
plot_figure_setup(hline=True, vline=True)
